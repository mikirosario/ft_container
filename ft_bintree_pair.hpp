...
/*   Updated: 2021/11/28 03:25:30 by miki             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
namespace ft
{
	...
	class bintree_pair : public ft::Abintree< ft::pair<T1, T2>, T1, T2, Compare, Alloc >, /*DEBUG*/public ft::bintree_printer< typename ft::Abintree< ft::pair<T1, T2>, T1, T2, Compare, Alloc >::t_bstnode, int/*DEBUG*/ >
	{
		...
		public:
			...
			bintree_pair(void) : Abintree<data_type, key_type, mapped_type, key_compare, allocator_type>() {};
			~bintree_pair(void) {
				//debug
			...
			*/
			...
			iterator	insert(iterator hint, value_type const & data) {
				if (this->is_valid_position(--hint, data.first))
				{
					...
					//DEBUG	
					return (iterator(this->bintree_add(node, data, data.first))); //constant time insertion
				}
				//DEBUG
			...
			*/
			...
			void		clear(void) {
				this->_root = this->bintree_free(this->_root);
			}
